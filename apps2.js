...
function show(user) {
  ...
  const childHTML = `<li id=${user._id}> ${user.amount} - ${user.dish} - ${user.select}
                          <button onclick=deleteUser('${user._id}') class=delete> Delete User </button>
                          <button onclick=editUser('${user.amount}','${user.dish}','${user.select}','${user._id}') class=edit>Edit User </button>
                       </li>`;
...
function editUser(amount, des, select, userId) {
  ...
  document.getElementById("dish").value = price;
